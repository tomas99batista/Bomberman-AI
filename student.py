...
def best_spot_wall(mapa, wall, actual_position):
    ...
    up = True if y_wall + \
        1 != 0 and not Map.is_blocked(mapa, [x_wall, y_wall + 1]) else False
    down = True if y_wall - \
        1 != 0 and not Map.is_blocked(mapa, [x_wall, y_wall-1]) else False
    left = True if x_wall - \
        1 != 0 and not Map.is_blocked(mapa, [x_wall - 1, y_wall]) else False
    right = True if x_wall + \
        1 != 0 and not Map.is_blocked(mapa, [x_wall + 1, y_wall]) else False
    print(f'UP: {up}, DOWN: {down}, LEFT: {left}, RIGHT: {right}.')
    min_hypot = sys.maxsize
    bomb_spot = ""  # The spot where the bomb will be placed
    # The coordinates where the bomb will be placed
    bomb_spot_coords = [[0, 0]]
...
class Celulas(SearchDomain):
    ...
    if up:
        ...
        print(
            f'Bomb_Spot: {bomb_spot}, Bomb_Spot_Coordinates: {bomb_spot_coords}, Hyp: {min_hypot}')
    ...
    async def agent_loop(server_address="localhost:8000", agent_name="student"):
        async with websockets.connect(f"ws://{server_address}/player") as websocket:
            ...
            mapa = Map(size=game_properties["size"],
    ...
    while True:
                try:
                    ...
                    bomb_spot_coords = best_spot_wall(
                        mapa, best_wall, actual_position)
